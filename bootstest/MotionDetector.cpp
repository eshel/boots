...
// comment-out to add kalman filter-like orientation calculation
//#define CALCULATE_ORIENTATION

#define ACCELEROMETER_READING_TO_WORLD_RATIO (8.0f/32768.0f)
...
{
	static MotionDetector MotionDetector;
...
// The full-scale range of the accelerometer: 0 = +/- 2g, 1 = +/- 4g, 2 = +/- 8g, 3 = +/- 16g
  mpu.setFullScaleAccelRange(8); // set accelerometer to 8 g range
  ...
  float acceleration = 0.0f;
     if(mpu.getIntDataReadyStatus() == 1) { // wait for data ready status register to update all data registers
       ...
       // read the raw sensor data
        ...
        acceleration = sqrt((int32_t)a1*a1 + (int32_t)a2*a2 + (int32_t)a3*a3);
        this->acceleration.addValue(acceleration);
        ax = a1*ACCELEROMETER_READING_TO_WORLD_RATIO; // 8 g full range for accelerometer
        ay = a2*ACCELEROMETER_READING_TO_WORLD_RATIO;
        az = a3*ACCELEROMETER_READING_TO_WORLD_RATIO;
...
//  the maxMotionDetectorm and minMotionDetectorm readings (generally achieved at the North magnetic direction). The average of the sum divided by two
...
// MahonyQuaternionUpdate(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, my, mx, mz);
    ...
    if (delt_t > 10) { // update LCD once per half-second independent of read rate
      Serial.print((unsigned long)now); Serial.print(',');
      ...
      Serial.println( acceleration*ACCELEROMETER_READING_TO_WORLD_RATIO);
#ifdef CALCULATE_ORIENTATION      
    ...
    // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
      ...
      // an ODR of 10 Hz for the magnetometer produce the above rates, maxMotionDetectorm magnetometer ODR of 100 Hz produces
